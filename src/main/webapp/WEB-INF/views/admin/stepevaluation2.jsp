        ...
        <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="passForm" method="post" action="/updatePass2">
                                ...
                                <tr>
                                    ...
                                    <td class="pass-toggle">
                                        <div class="toggle-btn ${stageApplicant.secondPassYn eq 'Y' ? 'active' : 'inactive'}"
                                             ...
                                             onclick="togglePass('${stageApplicant.aplNo}', '${stageApplicant.secondPassYn}')"></div>
                                        <!-- 각 지원자의 합격 여부를 서버로 전송할 hidden input -->
                                        <input type="hidden" id="secondPassYn_${stageApplicant.aplNo}" name="secondPassYn" value="${stageApplicant.secondPassYn}">
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
...
<script>
    ...
    function executeEvaluation() {
        ...
        xhr.open("POST", "/executeEvaluation2", true);
...
<script>
    function togglePass(aplNo, currentsecondPassYn) {
        // 합격 여부를 반전
        var newsecondPassYn = currentsecondPassYn === 'Y' ? 'N' : 'Y';

        // input 요소의 값을 변경
        var secondPassYnInput = document.getElementById("secondPassYn_" + aplNo);
        secondPassYnInput.value = newsecondPassYn;

        // AJAX 요청을 통해 서버에 업데이트
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/updatePass2", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function() {
            if (this.readyState === XMLHttpRequest.DONE) {
                if (this.status === 200) {
                    ...
                    toggleBtn.classList.toggle('active', newsecondPassYn === 'Y');
                    toggleBtn.classList.toggle('inactive', newsecondPassYn !== 'Y');
        ...
        xhr.send("aplNo=" + encodeURIComponent(aplNo) + "&secondPassYn=" + encodeURIComponent(newsecondPassYn));
    }

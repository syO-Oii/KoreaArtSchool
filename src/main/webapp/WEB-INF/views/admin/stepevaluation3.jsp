    ...
    <ul class="sidebar-nav" id="sidebar-nav">
        ...
        <li class="nav-item">
            ...
            <ul id="charts-nav" class="nav-content " data-bs-parent="#sidebar-nav">
                ...
                <li>
                    <a href="/admin/stepevaluation3"  class="active" >
                                ...
                                <tr>
                                    ...
                                    <td class="pass-toggle">
                                        <div class="toggle-btn ${stageApplicant.finalPassYn eq 'Y' ? 'active' : 'inactive'}"
                                             ...
                                             onclick="togglePass('${stageApplicant.aplNo}', '${stageApplicant.finalPassYn}')"></div>
                                        <!-- 각 지원자의 합격 여부를 서버로 전송할 hidden input -->
                                        <input type="hidden" id="finalPassYn_${stageApplicant.aplNo}" name="finalPassYn" value="${stageApplicant.finalPassYn}">
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
...
<script>
    ...
    function executeEvaluation() {
        ...
        xhr.open("POST", "/executeEvaluation3", true);
...
<script>
    function togglePass(aplNo, currentfinalPassYn) {
        // 합격 여부를 반전
        var newfinalPassYn = currentfinalPassYn === 'Y' ? 'N' : 'Y';

        // input 요소의 값을 변경
        var finalPassYnInput = document.getElementById("finalPassYn_" + aplNo);
        finalPassYnInput.value = newfinalPassYn;

        // AJAX 요청을 통해 서버에 업데이트
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/updatePass3", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function() {
            if (this.readyState === XMLHttpRequest.DONE) {
                if (this.status === 200) {
                    ...
                    toggleBtn.classList.toggle('active', newfinalPassYn === 'Y');
                    toggleBtn.classList.toggle('inactive', newfinalPassYn !== 'Y');
        ...
        xhr.send("aplNo=" + encodeURIComponent(aplNo) + "&finalPassYn=" + encodeURIComponent(newfinalPassYn));
    }
